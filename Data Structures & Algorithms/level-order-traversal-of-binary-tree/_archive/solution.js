 ...
 * - Beat `submission-1` (now archived) this round: that one was a
 *   conventional BFS but used `queue.shift()` inside the level loop,
 *   which is O(n) per call and degrades the traversal to O(n^2) in
 *   practice. This DFS+Map approach is genuinely O(n), so it wins on
 *   complexity despite the leftover console.log and unconventional
 *   shape.
 *
